...
T: cmd=clojure -M:test -m cognitect.test-runner ; reload-before-test=require:reload ; cover≈49t/257a
...
I: inv={determinism∀f, iface-consistency, time↑only, timers→FIFO-promote→random-select, completion→micro-q, schedule→select if >1, cancel→sync(match-missionary), thread-capacity-respected}
