...
D: state{:now-ms :next-id :micro-q :timers :trace :driver-thread :schedule-idx} ; policy∈{:fifo,:seeded} ; schedule=nil|vec(decisions)
...
T: cmd=clojure -M:test -m cognitect.test-runner ; reload-before-test=require:reload ; cover≈35t/190a
...
V: vars{*scheduler* *is-deterministic*}
E!: ex-data[:mt/kind]∈{::deadlock ::budget-exceeded ::off-scheduler-callback ::illegal-transfer ::no-scheduler} ; ::idle=return
API:
  make=(make-scheduler [opts])
  read=(now-ms pending trace)
...
SAFE: read-only={now-ms pending trace done? ready? terminated?}
